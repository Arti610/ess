...
const EditUserForm = () => {
  ...
  const handlePress = async () => {
    try {
      ...
      if (selectedUsertype === 'Staff') {
        fData.append('manager', selectManager ? selectManager : null);
      }
      ...
      fData.append('location_master', selectedLocation ? selectedLocation : null);
      fData.append('week_off', selectWeekoff ? selectWeekoff : null);
      if (selectShiftTime != null) {
        fData.append('branch_info', branchInfo.id ? branchInfo.id : null);
      }

      const res = await updateApi.updateUser( userData.id, fData, {
        headers: {
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        navigation.navigate('UserProfile', {userData: res.data, id : id,  userData : userData});
        Toast.show({
          type: 'success',
          position: 'top',
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await getApi.getIndividualUser(userData.id);

        if (res.data) {
          setFormdata({
            ...formdata,
            profile_image: res.data.user_data.profile_image
              ? res.data.user_data.profile_image
              : null,
            ...
            phone_number: res.data.user_data.phone_number
              ...
              : null,
            gender: res.data.user_data.gender
              ? res.data.user_data.gender
              : null,
            manager: res.data.user_data.manager
              ? `${res.data.user_data.manager.first_name} ${res.data.user_data.manager.last_name}`
              : null,
            department: res.data.user_data.department
              ? res.data.user_data.department.name
              : null,
            designation: res.data.user_data.designation
              ? res.data.user_data.designation.name
              : null,
            location_master: res.data.user_data.location_master
              ? res.data.user_data.location_master.selectd_place
              : null,
            user_type: res.data.user_data.user_type
              ? res.data.user_data.user_type
              : null,
            week_off: res.data.user_data.week_off
              ? res.data.user_data.week_off.name
              : null,
            address: res.data.user_data.address
              ? res.data.user_data.address
              : null,
            check_in_time: res.data.user_data.check_in_time
              ? res.data.user_data.check_in_time
              : null,
          });
          setSelectedGender(
            res.data.user_data.gender ? res.data.user_data.gender : null,
          );
          setSelectedUsertype(
            res.data.user_data.user_type ? res.data.user_data.user_type : null,
          );
          setSelectManager(
            res.data.user_data.manager ? res.data.user_data.manager.id : null,
          );
          setSelectDepartment(
            res.data.user_data.department
              ? res.data.user_data.department.id
              : null,
          );
          setSelectDesignation(
            res.data.user_data.designation
              ? res.data.user_data.designation.id
              : null,
          );
          setSelectWeekoff(
            res.data.user_data.week_off ? res.data.user_data.week_off.id : null,
          );
          setSelectedLocation(
            res.data.user_data.location_master
              ? res.data.user_data.location_master.id
              : null,
          );
          ...
          if (res.data.user_data.branch_info) {
            const checkInTime = moment(
              res.data.user_data.branch_info.check_in_time,
              'HH:mm',
            ).format('hh:mm A');
            const checkOutTime = moment(
              res.data.user_data.branch_info.check_out_time,
              'HH:mm',
            ).format('hh:mm A');
            const shiftTimeData = `${checkInTime} to ${checkOutTime}`;
        ...
        console.log(
          'Error during fetch individual data details for filling up the form fields',
          error,
        );
      }
    };
      ...
      <View style={styles.formContainer}>
            ...
            <Image
              source={
                formdata.profile_image
                  ? {uri: `${API_CONFIG.imageUrl}${formdata.profile_image}`}
                  : require('../../../assests/userProfile.webp')
              }
              style={styles.updateProfile}
            />
        ...
        <View style={styles.inputContainer}>
          ...
          <SelectList
            ...
            save="value"
            placeholder={
              selectedGender ? selectedGender : 'Select Gender e.g. (Female)'
            }
          ...
          <SelectList
            ...
            placeholder={
              formdata.designation != null
                ...
                : 'Select Designation e.g. (Developer)'
            }
            notFoundText="Data not found"
        ...
        <View style={styles.inputContainer}>
          ...
          <SelectList
            ...
            placeholder={
              selectShiftTime
                ? selectShiftTime
                : 'SelectShift Time e.g. (10:00 AM to 06:00 PM'
            }
            notFoundText="Data not found"
        ...
        <View style={styles.inputContainer}>
          ...
          <SelectList
            ...
            save="key"
            placeholder={
              formdata.location_master != null
                ? formdata.location_master
                : 'Select Department e.g. (IT)'
            }
            notFoundText="Data not found"
