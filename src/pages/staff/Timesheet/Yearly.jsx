...
const Yearly = () => {
    ...
    const filterData = (status) => {
        ...
        if (status === 'All') {
            ...
            newData = data.filter(item => item.attendance === status && moment(item.date).year() === currentYear);
        }
        ...
        <>
            ...
            <View style={style.container}>
                ...
                <TouchableOpacity onPress={() => handleFilterData('A')}>
                    <Text style={status === 'A' ? style.inactive : style.active}>Absent {`(${data && data.filter(item => item.attendance === 'A' && moment(item.date).year() === moment().year()).length})`}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => handleFilterData('W')}>
                    <Text style={status === 'W' ? style.inactive : style.active}>Week Off {`(${data && data.filter(item => item.attendance === 'W' && moment(item.date).year() === moment().year()).length})`}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => handleFilterData('H')}>
                    <Text style={status === 'H' ? style.inactive : style.active}>Holiday {`(${data && data.filter(item => item.attendance === 'H' && moment(item.date).year() === moment().year()).length})`}</Text>
                </TouchableOpacity>
            </View>
